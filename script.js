...
function renderProducts(list = products) {
  ...
  container.innerHTML = list.map(p => {
    if (p.images) {
      let imagesHtml = p.images.map((img, idx) => 
        `<img src="${img}" alt="${p.name}" style="width:100%; height:100%; object-fit:cover; display:${idx === 0 ? 'block' : 'none'};" class="product-slide-${p.id}">`
      ).join("");
...
function openProductModal(id) {
  ...
  let contentHtml = "";
  if (p.images) {
    let imgList = p.images.map((img, idx) => 
      `<img src="${img}" alt="${p.name}" style="width:100%; height:250px; object-fit:cover; display:${idx === 0 ? 'block' : 'none'};" class="modal-slide">`
    ).join("");
...
function renderCart() {
  ...
  el.innerHTML = cart.length ? cart.map((p, i) => {
    let displayThumb = p.images ? `<img src="${p.images[0]}" style="width:100%; height:100%; object-fit:cover;">` : (p.emoji || "🛍️");
  ...
  if (totalEl) totalEl.textContent = "₹" + cart.reduce((s, p) => s + p.price, 0);
}
...
// 4. Checkout System (WhatsApp & Formspree)
function checkout() {
...
function submitCustom(e) {
  ...
  customText += `*Design/Idea:* ${idea}%0A%0A`;
  customText += `_(Note: Reference image can be shared directly in chat if required.)_`;
  
  const whatsappNumber = "919204965346";
  const whatsappURL = `https://wa.me/${whatsappNumber}?text=${customText}`;
  
  const msgEl = document.getElementById("customMsg");
  if (msgEl) msgEl.textContent = "Opening WhatsApp to send your request...";
